...
function openOfferModal() {
  ...
  const content = document.getElementById('offerText');
  ...
  fetch('offer.html')
    ...
    .catch(() => {
      // Если fetch не сработал — вставляем резервный текст прямо в коде
      content.innerHTML = `
        <h1>Публічна оферта</h1>
        <p>Цей документ є офіційною пропозицією ФОП Стецуріна Ірина Олександрівна...</p>
        <p style="color:#c00; margin-top:40px;">Не вдалося завантажити документ. Відкрийте сайт через сервер (не через file://) або зверніться до адміністратора.</p>
      `;
    });
}
